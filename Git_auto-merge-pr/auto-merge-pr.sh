...
then
	...
	then
        ...
        echo "$repo_dir"
        cd "$repo_dir"
		echo "merge failed: $merge_result, please resolve conflict manaully in path: $repo_dir\
        continue merge after fixed by 'git merge --continue', then run this shell agagin!👮👮👮⚠️⚠️⚠️"
		exit
	fi
