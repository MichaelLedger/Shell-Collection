...
function createImageSelector() {
    ...
    input.type = 'file';
    input.accept = 'image/png'; //'image/png, image/jpeg, image/jpg, image/gif, image/webp';
    return input;
}
...
function drawCorners() {
    ...
    corners.forEach((corner, index) => {
        ...
        // Alternate between two colors based on blinkState
        interactiveCtx.fillStyle = blinkState ? '#00FF00' : '#FFFF00';
        interactiveCtx.fill();
        interactiveCtx.strokeStyle = '#000000';
        interactiveCtx.lineWidth = 2;
        interactiveCtx.stroke();

        // Add drag indicator lines inside the circle
        interactiveCtx.beginPath();
        interactiveCtx.moveTo(corner.x - 4, corner.y - 4);
        interactiveCtx.lineTo(corner.x + 4, corner.y + 4);
        interactiveCtx.moveTo(corner.x - 4, corner.y + 4);
        interactiveCtx.lineTo(corner.x + 4, corner.y - 4);
        interactiveCtx.strokeStyle = '#333333';
        interactiveCtx.lineWidth = 1.5;
        interactiveCtx.stroke();
    });
}

// Add this function to start the blinking animation
function startBlinking() {
    if (blinkInterval) clearInterval(blinkInterval);
    
    // Blink every 500ms (half a second)
...
// Add this function to stop blinking
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    analyzeButton.addEventListener('click', () => {
        if (!selectedImage) {
            showToast('Please choose a png first!');
            return;
        }
    ...
    confirmButton.addEventListener('click', () => {
        ...
        // Show the toast notification with instructions
        showToast('Drag the blinking green control points to adjust the area, then calculate margins.');
    });

    // 添加计算边距按钮的点击事件
    calculateMarginsButton.addEventListener('click', () => {
        if (!corners || corners.length !== 4 || !finalRect) {
            showToast('Please detect the transparent area and confirm the rectangle first!');
            return;
        }
        ...
        if (!offsets.every(offset => offset.x == 0 && offset.y == 0)) {
            ...
            // Get the basic PO info without margins
            let basicPoInfo = poInfo.textContent;
            if (basicPoInfo.includes("margins")) {
                basicPoInfo = basicPoInfo.substring(0, basicPoInfo.indexOf(', "margins"'));
            }
            
            // Update PO info with new margins
            poInfo.textContent = basicPoInfo + `, "margins": [
            ...
            // Get the basic template info without margins
            let basicTemplateInfo = templateInfo.textContent;
            if (basicTemplateInfo.includes("margins")) {
                basicTemplateInfo = basicTemplateInfo.substring(0, basicTemplateInfo.indexOf(', "margins"'));
            }
            
            // Update template info with new margins
            templateInfo.textContent = basicTemplateInfo + `, "margins": [
           ...
           // Show toast to confirm margins were updated
            showToast('Margins updated successfully!'); 
        } else {
            showToast('No margin adjustments detected.');
        }
    });
});