...
def merge_csv_files():
    ...
    def sort_key(row):
        ...
        try:
            # Return tuple (0, int) for numeric names - sorts first
            numeric_part = name.replace('.jpg', '').replace('.jpeg', '').replace('.png', '')
            return (0, int(numeric_part))
        except:
            # Return tuple (1, str) for non-numeric names - sorts after
            return (1, name)
